...
def process_coordinates(coords, padded_image, dims: ImageDimensions, previous_box=None):
    ...
    try:
        ...
        x1, y1 = round(coords[0] / 896.0 * dims.padded_w), round(
            coords[1] / 896.0 * dims.padded_h
        )
        x2, y2 = (
            round(coords[2] / 896.0 * dims.padded_w) + 1,
            round(coords[3] / 896.0 * dims.padded_h) + 1,
        )
...
def crop_margin(img: Image.Image) -> Image.Image:
    ...
    try:
        ...
        if w > 0 and h > 0:
            cropped_img = img.crop((a, b, a + w, b + h))
            # 添加这一行：它会调用系统默认的图片查看器打开图像
            # cropped_img.show(title="裁剪后的结果")
            return cropped_img
        return img
    except Exception as e:
...
def visualize_layout(
    image_path, layout_results, save_path, alpha=0.3, original_image=None
):
    ...
    for idx, (bbox, label) in enumerate(layout_results):
        ...
        # Draw text
        cv2.putText(
            image, label_text, (text_x, text_y), font, font_scale, (0, 0, 0), thickness
        )
...
def save_layout_json(layout_results, image_path, save_dir, original_image=None):
    ...
    layout_data = {
        "image": (
            os.path.basename(image_path) if isinstance(image_path, str) else "pdf_page"
        ),
    ...
    base_name = os.path.splitext(
        os.path.basename(image_path) if isinstance(image_path, str) else "page"
    )[0]
