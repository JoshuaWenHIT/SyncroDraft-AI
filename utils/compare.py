...
def compare_view_annotations(
    a_annos: List[Dict[str, Any]], b_annos: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for a_anno in a_annos:
        ...
        view_diff["match_candidates"][a_uid][
            "same_direction_candidates"
        ] = same_direction_candidates.copy()
        ...
        for idx, b_anno in enumerate(b_annos_copy):
            if (
                b_anno in same_direction_candidates
                and b_anno.get("content") == a_content
            ):
        ...
        if match_index >= 0:
            ...
            if match_type == "full_match":
                view_diff["matched"].append(
                    {"a_annotation": a_anno, "b_annotation": matched_b_anno}
                )
...
def cross_view_match(
    a_unmatched: List[Dict[str, Any]], b_unmatched: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for a_anno in a_unmatched_raw:
        ...
        same_category_candidates = [
            b for b in b_unmatched_copy if b.get("category") == a_category
        ]
...
def generate_diff_report(
    a_file: str, b_file: str, output_file: str = "diff_report.json"
) -> None:
    ...
    for view in all_views:
        ...
        diff_report["summary"]["same_view_content_diff"] += len(
            view_diff["content_different"]
        )
        ...
        if any(
            [
                len(view_diff[key]) > 0
                for key in ["content_different", "a_only", "b_only"]
            ]
        ):
        ...
        diff_report["diff_details"]["same_view_content_different"].extend(
            view_diff["content_different"]
        )
    ...
    diff_report["summary"]["cross_view_matched"] = len(
        cross_result["cross_view_matched"]
    )
    diff_report["summary"]["final_a_only_annotations"] = len(
        cross_result["final_a_only"]
    )
    diff_report["summary"]["final_b_only_annotations"] = len(
        cross_result["final_b_only"]
    )
    ...
    diff_report["diff_details"]["cross_view_matched"].extend(
        cross_result["cross_view_matched"]
    )
    ...
    # 控制台输出（突出溯源关键信息）
    log_utils.log(f"✅ 差异比对报告生成完成！")
    log_utils.log(f"📊 比对汇总（溯源重点）：")
    log_utils.log(f"   - 总比对View数：{diff_report['summary']['total_views']}")
    log_utils.log(
        f"   - 同View内容差异数：{diff_report['summary']['same_view_content_diff']}（需核对content）"
    )
    log_utils.log(
        f"   - 跨View匹配数：{diff_report['summary']['cross_view_matched']}（view不同但内容一致）"
    )
    log_utils.log(
        f"   - 最终A独有数：{diff_report['summary']['final_a_only_annotations']}（无匹配，需重点溯源）"
    )
    log_utils.log(
        f"   - 最终B独有数：{diff_report['summary']['final_b_only_annotations']}（无匹配，需重点溯源）"
    )
    log_utils.log(f"📄 报告路径：{output_file}")
    log_utils.log(f"🔍 溯源关键字段：diff_details下的trace_info（uid/view/bbox）")
