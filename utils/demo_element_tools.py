...
class DOLPHIN:
    def __init__(self, model_id_or_path="./hf_model"):
...
def recognize_element(image_path, element_type="text", ocr_model=None, save_dir=None):
    """Recognize element in image through function call (not command line)

    Args:
        image_path (str): Path to the input image
        element_type (str): Type of element to process (text, table, formula, code)
        ocr_model (DOLPHIN): Initialized OCR model instance
        save_dir (str): Directory to save results (optional)

    Returns:
        tuple: (result_text, recognition_result_dict)
    """
    # Validate inputs
    if not os.path.exists(image_path):
        raise FileNotFoundError(f"Input image path {image_path} does not exist")

    if element_type not in ["text", "table", "formula", "code"]:
        raise ValueError("element_type must be one of: text, table, formula, code")
