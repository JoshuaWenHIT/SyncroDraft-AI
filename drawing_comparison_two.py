...
from utils.image_preprocess import image_preprocess_2
from utils.PMI_extract import PMI_extract_2
from tools.merge_json import merge_main
from utils.compare import generate_diff_report
from utils.json_to_excel import json_to_excel_2
from tools.json_vis import json_vis_2
...
if __name__ == '__main__':
    ...
    # 输入两张对比图像处理，将工程图纸中视图的部分分割并预处理，给第二阶段目标检测使用
    # 同时，将两张工程图纸对应的三视图和截面图比对相似度，并保存到csv文件
    image_preprocess_2(args.image_path1, args.image_path2, args.output_dir, args.batch_predict, args.alignment_threshold, args.similarity_threshold)
    ...
    PMI_extract_2(image_fold1, image_fold2, args.output_dir)
    ...
    report_json_path = os.path.join(json_results_path, "{}_compare_report.json".format(image_prefix))
    ...
    json_to_excel_2(
        ...
        excel_file=os.path.join(final_results_path, "{}_merged.xlsx".format(image_prefix))
    )
    ...
    json_vis_2(report_json_path, image_folder, output_folder, position_file_folder)
    end_time = time.time()
